...
# Fix for endless messages
USE_REGION_ITER = True
...
class GlText(Gl):
    ...
    def __init__(self,
        ...
        """
            ...
            unit_mode : ['ADAPTIVE','METER','CENTIMETER','MILLIMETERS','FEET','INCH','RADIANS','DEGREE']
        ...
        self.unit_separate = unit_separate
    ...
    def label(self, label):
        _label = self.translate(label)
        self._label = "%s%s%s" % (self.prefix, _label, self.postfix)

    def translate(self, label):
        _label = bpy.app.translations.pgettext(label, "archipack").split(" ")
        return " ".join([bpy.app.translations.pgettext(txt, "archipack") for txt in _label])
    ...
    def decimal(self, val, precision):
        _v = Decimal(val).__format__(".{}f".format(precision))
        return self.remove_exponent(Decimal(_v))

    def add_units(self, context):
        if self.value is None:
            return ""
        system = context.scene.unit_settings.system
        use_separate = self.unit_separate
        if self.unit_type == 'ANGLE':
            ...
            if mode == 'AUTO':
                ...
                use_separate = context.scene.unit_settings.use_separate
        ...
        if mode == 'METERS':
            if use_separate:
                ...
                m = val - cm
                cm *= 100
                return "{}m {}cm".format(self.decimal(m, 0), self.decimal(cm, 1))
            unit = "m"
        elif mode == 'CENTIMETERS':
            val *= (100 ** self.dimension)
            unit = "cm"
        elif mode == 'MILLIMETERS':
            ...
            if use_separate:
                ...
                feet = val - inches
                inches *= 12
                return "{}' {}\"".format(self.decimal(feet, 0), self.decimal(inches, self.precision))
            unit = "'"
        elif mode == 'INCHES':
        ...
        return "{} {}".format(self.decimal(val, self.precision), unit)
    ...
    def draw(self, context, render=False):
        # print("draw_text %s %s" % (, type(self).__name__))
        self.render = render
        p = self.position_2d_from_coord(context, self.pts[0], render)
        if not self.can_draw:
            return

        # print("Draw() ", self.text, self.__class__.__name__)
...
class GlBaseLine(Gl):
    ...
    def draw(self, context, render=False):
        ...
        for pt in self.pts:
            ...
            if not self.can_draw:
                return
            list_verts_co.append(tuple(p[0:2]))
...
class GlImage(Gl):
    ...
    def draw(self, context, render=False):
        ...
        g_image.draw(self.image, coords)
...
class ThumbHandle(GlHandle):
    ...
    def __init__(self, size_2d, label, image=None, draggable=False, selectable=False, d=2):
        ...
        self.overlay = GlPolygon((0.0, 0.0, 0.0, 0.3), d=2, n_pts=4)
    ...
    def set_pos(self, context, pos_2d):
        ...
        self.overlay.set_pos([p0, Vector((p1.x, p0.y)), p1, Vector((p0.x, p1.y))])
    ...
    def draw(self, context, render=False):
        ...
        if not self.hover:
            self.overlay.draw(context, render=False)
...
class Screen():
    ...
    def size(self, context):
        ...
        if system.use_region_overlap and USE_REGION_ITER:
            #    system.window_draw_method in {'TRIPLE_BUFFER', 'ADAPTIVEMATIC'}):
            area = context.area
            regions = area.regions
            # iter over regions lead to py_rna enum error as a region type is ""
            for r in regions:
                if r.type == 'TOOLS':
                    x_min += r.width
                elif r.type == 'UI':
...
class FeedbackPanel():
    ...
    def instructions(self, context, title, explanation, shortcuts):
        ...
        for key, _label in shortcuts:
            ...
            label = GlText(d=2, label=_label, prefix=' : ',
                           font_size=prefs.feedback_size_shortcut,
