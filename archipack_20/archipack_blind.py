...
class archipack_blind(ArchipackObject, Manipulable, PropertyGroup):
    tabs: EnumProperty(
        ...
        items=(
            ...
            ('PARTS', 'Parts', 'Display parts settings', 'NONE', 1),
    ...
    def update(self, context, bend=False):
        ...
        bmed.buildmesh(o, verts, faces, matids, weld=False)

        if bend and o.parent is not None:
            p = o.parent
            if "archipack_window" in p.data:
                d = p.data.archipack_window[0]
                if len(d.bend) > 0:
                    bend = d.bend[:]
                    bend.sort(key=lambda x: x.x)
                    for b in bend:
                        if b.x < 0:
                            d.soft_segment(p, o, b.a, b.x)
...
class ARCHIPACK_PT_blind(ArchipackPanel, Archipacki18n, Panel):
    ...
    def draw(self, context):
        ...
        # Presets operators
        self.draw_op(context, layout, row, "archipack.blind_preset_menu",
                     text=bpy.types.ARCHIPACK_OT_blind_preset_menu.bl_label, icon="PRESET")
        self.draw_op(context, layout, row, "archipack.blind_preset", icon='ADD', text="")
        self.draw_op(context, layout, row, "archipack.blind_preset", icon='REMOVE', text="").remove_active = True
        ...
        if d.tabs == 'MAIN':
            self.draw_prop(context, layout, box, d, 'style')
            ...
            self.draw_prop(context, layout, box, d, 'ratio')

        elif d.tabs == 'PARTS':
            self.draw_label(context, layout, box, "Slat")

            if d.style == 'JAPANESE':
                self.draw_prop(context, layout, box, d, 'panels')

            else:
                if d.style != 'ROLLER':
                    self.draw_prop(context, layout, box, d, 'y')

                if d.style in {'ROLLER', 'BLADES'}:
                    self.draw_prop(context, layout, box, d, 'blind_angle')

                elif d.style != 'PLATED':
                    self.draw_prop(context, layout, box, d, 'angle')

            box = layout.box()
            self.draw_prop(context, layout, box, d, 'frame_enable')
            if d.frame_enable:
                self.draw_prop(context, layout, box, d, 'frame_height')
                self.draw_prop(context, layout, box, d, 'frame_depth')
...
class ARCHIPACK_OT_blind_preset_create(PresetMenuOperator, Operator):
    bl_description = "Show Blind presets and create object at cursor location"
...
class ARCHIPACK_OT_blind_preset_menu(PresetMenuOperator, Operator):
    bl_description = "Create Blind from preset"
...
class ARCHIPACK_OT_blind_preset(ArchipackPreset, Operator):
    bl_description = "Add / remove a  Blind Preset"
